// --------------------------------------------------------- 
// Definition of the HIAFSimpleDetectorConstruction class
...
//#include "HIAFSimpleMaterialManager.hh"
//#include "HIAFSimpleFieldSetup.hh"
//#include "HIAFSimpleCommon.h"
...
class HIAFSimpleDetectorConstruction : public G4SolVDetectorConstruction
{
private:
  ...
  void ConstructEndFMF2(G4double AbsPos);
  void ConstructDipole(G4double AbsPos);
  void ConstructLastStations(G4double AbsPos);
...
public:
  ...
  const double CD_Rmax = 600.;
  const double CD_Rmin = 40.;
  const double CD_Z = 2000.;

  const double Solenoid_RadiusMin = 780. ; //mm
  const double Solenoid_RadiusMax = 900. ; //mm
  const double Solenoid_Length = 2000. ; //mm

  const double MDC_RadiusMin = 40.;
  const double MDC_RadiusMax = 600.;
  const double MDC_LayerSpacing = 3.5;
  const int MDC_NbLayer = 17;
  
  const double PSB_Length = 1900.;
