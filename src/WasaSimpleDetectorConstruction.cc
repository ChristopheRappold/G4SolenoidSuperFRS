...
void WasaSimpleDetectorConstruction::ConstructCD(G4double cd_rmax,G4double cd_z, G4double cdsPos_x, G4double cdsPos_y, G4double cdsPos_z)
{
  G4NistManager* materialMgr = G4NistManager::Instance();
  ...
  if(DoForRoot)
    {
      ...
      CD_physOutRoot = new G4PVPlacement(0, G4ThreeVector(cdsPos_x, cdsPos_y, cdsPos_z), CD_logOutRoot, "CDR", experimentalHall_logOutRoot, false,0);
  ...
  if(DoModHypHI==false)
  ...
  else
    {
      ...
      HypHI_Endcap_phys = new G4PVPlacement(0, G4ThreeVector(cdsPos_x, cdsPos_y, cdsPos_z+(cd_z+12.*cm)), HypHI_Endcap_log, "HypHI_Endcap",
					    experimentalHall_log, false,0);
      if(DoForRoot)
	AllPlacements.emplace_back(new G4PVPlacement(0, G4ThreeVector(cdsPos_x, cdsPos_y, cdsPos_z+(cd_z+12.*cm)), HypHI_Endcap_log, "HypHI_EndcapR",
  ...
  for (G4int i=0; i<MDC_NbLayer; i++)
	...
	rmin -= MDC_LayerSpacing;
      ...
      AllPlacements.emplace_back(new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),MDC_Setlog[i],nameSetPhys,MDC_body_log,false,i));
      
      MDC_tube[i] = new G4Tubs(name_sol, rmin, rmax, mdc_z, -0.5*CLHEP::twopi/Nb_MDC_cell, CLHEP::twopi/Nb_MDC_cell);
  ...
  if(DoForRoot)
      ...
      for(int i=0; i<MDC_NbLayer; ++i)
	{
	  ...
	  //MDC_Setlog[i]->SetVisAttributes(G4VisAttributes::Invisible);
	  AllPlacements.emplace_back(new G4PVPlacement(0,G4ThreeVector(0.,0.,0.),MDC_SetlogOutRoot[i],nameSetPhys,MDC_body_logOutRoot,false,i));
	}
    }
...
void WasaSimpleDetectorConstruction::ConstructDownTracker(G4double cd_z, G4double AbsPos, G4double TargetPos_x, G4double TargetPos_y, G4double TargetPos_z)
{
  G4NistManager* materialMgr = G4NistManager::Instance();
  ...
  G4int nb_panel = 16;
  G4VSolid* HypHI_SiliciumSeg = new G4Tubs("HypHI_SiSeg",0.*cm,4.*cm, 3.*mm,
