...
G4SolRunData::G4SolRunData(const G4String& name)
    : G4Run(), namefile(name), fileOut(nullptr), Tree(nullptr), LookCheckFile(false), LookCheckTree(false),
      CloseDone(false)
{
...
G4SolRunData::~G4SolRunData() { Close(); }
...
void G4SolRunData::FillPerEvent(const G4Event* event)
{
  ...
  const G4SolStackingAction* stackingUser =
      dynamic_cast<const G4SolStackingAction*>(G4RunManager::GetRunManager()->GetUserStackingAction());
  ...
  if(prim_id.size() != 0)
    {
	  ...
	  G4cout<< G4endl;
      ...
      else
        {
          ...
          const G4SolStacking::Daugthers_Info tempDaugther =
              stackingUser->Get_DaugthersInfo(fEvent->BeamTrackID[MotherID]);
